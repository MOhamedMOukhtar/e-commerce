...
import { getSmallStorageOrganisers } from "@/lib/firestore/folder/smallStorageOrganisers";
import { TProduct } from "@/types/product/product";
import { useEffect, useState } from "react";

function SmallStorageOrganisers() {
  const [products, setProducts] = useState<TProduct[]>([]);
  const [visibleCount, setVisibleCount] = useState(16);

  useEffect(() => {
    async function fetch() {
      const products = (await getSmallStorageOrganisers()) as TProduct[];
  ...
  return (
    <div className="mx-12 mb-12">
      ...
      <h2 className="mt-20 border-b pb-6 text-2xl" id="products">
        Small storage & organisers Products
      </h2>
...
export default SmallStorageOrganisers;
