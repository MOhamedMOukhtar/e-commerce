...
function ProductCardSmallChild({
  ...
  async function handleFavorite() {
    ...
    if (!user) {
      ...
      if (localFavorites?.length === 1) {
        ...
        setIsLoading(false);
        toast.success(
          `${product.title} was added to your ${favoriteListsArray[0].listName}`,
        );
        return;
      }
  ...
  // remove from list if more than one list
  ...
  return (
    <>
      ...
      >
        ...
        <div
          onClick={(e) => e.stopPropagation()}
          className={`fixed top-0 right-[-15px] h-screen w-[460px] overflow-y-auto rounded-l-lg border border-black/30 bg-white transition duration-200 [scrollbar-gutter:stable] ${showInfo === "update" ? "translate-x-0" : "translate-x-full"} flex flex-col py-6 ps-6`}
        >
        ...
        >
          ...
          {(user ? favoritesLists : localFavorites)?.length === 10 ? null : (
            <Button
              variant={"border"}
