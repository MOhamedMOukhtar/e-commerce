...
function Product() {
  ...
  return (
    <div className="relative mx-12 mt-10">
      ...
      {/* Info side panel */}
      <div
        className={`fixed top-0 left-0 z-20 h-screen w-screen bg-black/30 transition duration-200 ${showInfo ? "" : "pointer-events-none"}`}
        style={{
      ...
      >
        <div
          ...
          className={`fixed top-0 right-0 z-30 h-screen w-1/3 overflow-y-auto rounded-l-lg border border-black/30 bg-white p-8 pt-24 pb-12 transition duration-200 [scrollbar-gutter:stable] ${showInfo ? "translate-x-0" : "translate-x-full"}`}
        >
          <button
            onClick={() => {
              setShowInfo(false);
          ...
          {detMeas === "details" ? (
            <div className="text-stone-700">
              ...
              <div>
                <div
                  className="relative cursor-pointer overflow-hidden py-8 hover:underline"
                  onClick={() => setShowMaterialsAndCare(!showMaterialsAndCare)}
                >
                  <h2 className="text-[15px] font-bold">Materials and care</h2>
                  <button className="absolute top-1/2 right-0 -translate-y-1/2 cursor-pointer">
                    <ChevronDownIcon
                      strokeWidth={2.5}
                      size={20}
                ...
                >
                  <div
                    dangerouslySetInnerHTML={{ __html: materialsAndCare || "" }}
                  />
                </div>
              </div>
            </div>
